...
<h2>Live link 🌍📡 - (https://movieapp-beta-ambervercel.app](https://movie-app-ai-j5jx.vercel.app/) </h2>
...
- 🤖 **NEW: AI-Powered Chatbot!**
    * **Conversational Recommendations:** Integrated a secure, real-time AI assistant for personalized movie and TV show suggestions.
    * **Secure Architecture:** The chatbot is powered by the **Google Gemini API**, utilizing **Vercel Serverless Functions** as a secure proxy to protect the API key.
    * **Rich Formatting:** Uses `react-markdown` to ensure AI responses (e.g., lists, bolding) are clean and readable.
...
- `useRef`
- `Custom-Hook`
- **`useChatbot`** (Logic for the new AI feature)
...
- **Axios (`axios`):** Efficient HTTP client for data retrieval.
- **Day.js (`dayjs`):** Date and time manipulation.
- **React (`react`) and React DOM (`react-dom`):** Core UI libraries.
- **Google Gemini API (`@google/genai`):** Powers the conversational AI assistant.
- **React Markdown (`react-markdown`):** Used to format AI text responses beautifully.
- **React Circular Progressbar (`react-circular-progressbar`):** Visually appealing progress bars.
- **React Icons (`react-icons`):** Diverse iconography.
- **React Infinite Scroll Component (`react-infinite-scroll-component`):** Seamless scrolling.
- **React Lazy Load Image Component (`react-lazy-load-image-component`):** Performance enhancement.
- **React Player (`react-player`):** Media player integration for trailers.
- **React Redux (`react-redux`):** Redux state integration.
- **React Router DOM (`react-router-dom`):** Application routing.
- **React Select (`react-select`):** Customizable select dropdowns.
- **Sass (`sass`):** CSS preprocessor for styling.
